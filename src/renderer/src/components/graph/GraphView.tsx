...
export default function GraphView(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const draw = (): void => {
      ...
      // The DOM chrome inherits dark mode through the inverted stone CSS
      // variables, but canvas strokes are literal colors — pick per frame so
      // a theme switch takes effect without a reload.
      ...
      for (const n of nodes) {
        ...
        if (n.id === sel?.id || n.id === hover?.id) {
          ctx.strokeStyle = dark ? '#f5f5f4' : '#292524'
